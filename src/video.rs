...
use crate::{PipelineData, PipelineState, Controller};
...
impl Widget<PipelineData> for VideoPlayer{
    fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, _event: &LifeCycle, data: &PipelineData, _env: &Env) {
        if data.state == PipelineState::Play{
    ...
    fn paint(&mut self, paint_ctx: &mut PaintCtx, _data: &PipelineData, _env: &Env) {
        ...
        if let Some(sample) = sample{
            ...
            paint_ctx.render_ctx.draw_image(&image, video_rect, InterpolationMode::Bilinear);
        }
    }

    fn update(&mut self, ctx: &mut UpdateCtx, _old_data: &PipelineData, _data: &PipelineData, _env: &Env) {
        ctx.children_changed(); // Hack to cause lifecycle event to start animation
        ctx.request_paint();
    }

    fn layout(&mut self, _ctx: &mut LayoutCtx, bc: &BoxConstraints, _data: &PipelineData, _env: &Env) -> Size {
        bc.max()
    }
